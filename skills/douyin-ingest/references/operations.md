...
6. **校验（实测重要）**：问问AI **并不总是听话**——口播/直播型视频会给逐字原文（✅），但**字幕/框架/概念型短视频（少连续口播）会无视提示词、直接返回结构化总结**（分点、`strong` 小标题、列表、概括式语言）。判别：出现"分点列表 / 加粗小标题 / 概括句式"且无口语连读特征 = **拿到的是 AI 总结，不是原话**。
   - 视为 Tier 1 失败。**绝不把 AI 总结当博主原话写进 investment-views**（违反"保留原话"铁律 + 反编造原则）。
   - 可强化提示词重试一次（"逐字、不要分点、无连续口播就说无连续口播"）；仍是总结 → 不降级凑数，直接 **Tier 3 `needs-manual`**，账本 `note` 记"问问AI 仅给总结非原话"，由用户人工决定是否以摘要形式入档。
   - 明显过短/空 → 同样视为 Tier 1 失败。
