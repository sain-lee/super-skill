...
5. **逐字稿即博主原话**：交给 Claude 按 Type D 结构化，**不要让问问AI 替你总结**（提示词已要求只出原文）。
6. **校验 + 折中分流（实测重要，用户 2026-05-18 定）**：问问AI 并不总听话——直播/口播型给逐字原文（✅）；**财报/研报/框架/概念型视频即便强化提示词重试也持续只返回结构化总结**（分点、`strong` 小标题、列表、概括句式）。判别：出现"分点列表/加粗小标题/概括句式"且无口语连读特征 = AI 总结非原话。按**折中政策分流**：
   - **逐字原话** → Type D 正常入「博主原话」区（标 `@博主`）。账本 `status:"integrated"`。
   - **强化提示词重试一次仍为结构化总结** → 不丢弃、不当原话：把总结作为 **『⚠️ 问问AI 摘要（非博主原话）』独立小节**写进 `investment-views` 按 Type D 路由的对应专题档，小节内显著标注"问问AI 生成·非博主原话 + 视频链接 + 时点"，与"博主原话"区**物理分开、绝不混标**。账本 `status:"integrated"`、`note` 含 `mode:ai-summary`。
   - **原话与总结都拿不到（过短/空/无内容）** → `needs-manual`，不写知识库、不编造。
   - 铁律不破：原话区只放原话；AI 摘要单列并标注来源——读者永远能分清哪句是博主说的、哪句是 AI 概括的。
